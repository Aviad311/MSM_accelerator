...
//       
//       Creation Date:  Sun Jun 28 00:23:36 2026
//       Version: 	r1p0
//
...
`ifdef POWER_PINS
module s8192x16 (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, RET1N);
`else
module s8192x16 (Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, RET1N);
`endif
  ...
  parameter MUX = 16;
  parameter MEM_WIDTH = 256; // redun block size 16, 128 on left, 128 on right
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
      ...
      if (WEN_int !== 1'b1) begin
        ...
        readLatch0 = {data_out[240], data_out[224], data_out[208], data_out[192], data_out[176],
...
`endif
 ...
 wire  CLK_;
  ...
  buf B46(D_[14], D[14]);
  buf B47(D_[15], D[15]);
  buf B48(EMA_[0], EMA[0]);
  buf B49(EMA_[1], EMA[1]);
  buf B50(EMA_[2], EMA[2]);
  buf B51(EMAW_[0], EMAW[0]);
  buf B52(EMAW_[1], EMAW[1]);
  buf B53(EMAS_, EMAS);
  buf B54(RET1N_, RET1N);

   `ifdef ARM_FAULT_MODELING
     s8192x16_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module s8192x16_error_injection (Q_out, Q_in, CLK, A, CEN, WEN);
   output [15:0] Q_out;
   input [15:0] Q_in;
   input CLK;
   input [12:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(13'd476,4'd6,2'd1,2'd0);
   `endif
end
   task add_fault;
