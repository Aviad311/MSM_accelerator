...
module tb_msm_axi_affine_multiwindow_dualclk_16win_1048576p_replay16_stageprofile_v1;
    ...
    `include "vectors/full_axi_affine_1048576p_replay16_stageprofile_v1_expected.svh"

    localparam int LOGICAL_POINTS_PER_WINDOW = MW_POINTS_PER_WINDOW;
    localparam int BASE_POINTS_PER_WINDOW = 65_536;
    localparam int REPLAY_FACTOR = 16;
    ...
    localparam int BASE_BEATS_PER_WINDOW =
        BASE_POINTS_PER_WINDOW * BEATS_PER_POINT;
    localparam int BASE_TOTAL_BEATS =
        NUM_WINDOWS * BASE_BEATS_PER_WINDOW;
    ...
    logic [511:0] axi_mem [0:BASE_TOTAL_BEATS-1];
    ...
    function automatic integer map_logical_to_base_beat(
        input integer logical_beat_index
    );
        integer logical_window;
        integer beat_within_large_window;
        integer beat_within_base_window;
        begin
            logical_window =
                logical_beat_index / BEATS_PER_WINDOW;
            beat_within_large_window =
                logical_beat_index % BEATS_PER_WINDOW;
            beat_within_base_window =
                beat_within_large_window % BASE_BEATS_PER_WINDOW;

            map_logical_to_base_beat =
                logical_window * BASE_BEATS_PER_WINDOW
                + beat_within_base_window;
        end
    endfunction
    ...
    initial begin : init_axi_memory
        integer beat;
        for (beat = 0; beat < BASE_TOTAL_BEATS; beat = beat + 1)
            axi_mem[beat] = '0;
        ...
        if (!axi_rst_n) begin
            ...
            if (m_axi_arvalid && m_axi_arready) begin
                ...
                if (m_axi_arsize !== 3'b110) begin
                    error_count++;
                    $error("[TB_STAGE1M_HB] bad ARSIZE=%0d", m_axi_arsize);
                end

                if (m_axi_arburst !== 2'b01) begin
                    error_count++;
                    $error("[TB_STAGE1M_HB] bad ARBURST=%0b", m_axi_arburst);
                end

                if (m_axi_araddr[5:0] != 0) begin
                    error_count++;
                    $error("[TB_STAGE1M_HB] unaligned ARADDR=%h", m_axi_araddr);
                end
            ...
            if (!m_axi_rvalid &&
                ...
                logical_index = (slave_addr_q - BASE_ADDR) >> 6;

                if ((logical_index < 0) ||
                    (logical_index >= TOTAL_BEATS)) begin
                    error_count++;
                    $error(
                        "[TB_STAGE1M_HB] invalid logical beat index=%0d",
                        logical_index
                    );
                    m_axi_rdata <= '0;
                end else begin
                    mem_index =
                        map_logical_to_base_beat(logical_index);
            ...
            if (m_axi_rvalid && m_axi_rready) begin
                ...
                if (m_axi_rlast !== (slave_beats_left_q == 1)) begin
                    error_count++;
                    $error("[TB_STAGE1M_HB] RLAST mismatch");
                end
                ...
                if (slave_beats_left_q == 1) begin
                    ...
                    if (gap_lfsr_q[8:6] == 3'b000) begin
                        ...
                        if ((logical_index < 0) ||
                            ...
                            error_count++;
                            $error(
                                "[TB_STAGE1M_HB] invalid next logical beat=%0d",
                                logical_index
                            );
                            m_axi_rdata <= '0;
                        end else begin
    ...
    always @(posedge msm_clk or negedge msm_rst_n) begin : msm_profile
        ...
        if (!msm_rst_n) begin
            ...
            if (busy && (msm_cycle_count >= next_heartbeat_cycle)) begin
                ...
                $display(
                    "[TB_STAGE1M_HB] HEARTBEAT=%0d msm_cycle=%0d window=%0d point_in=%0d mont_out=%0d build_fire=%0d completed=%0d fifo_stall=%0d build_done=%0b reduce_start=%0b reduce_done=%0b",
                    heartbeat_count + 1,
                    msm_cycle_count,
                    active_msm_window,
            ...
            if (busy && (window_index != last_seen_window)) begin
                ...
                $display(
                    "[TB_STAGE1M_HB] entered window=%0d base=%h msm_cycle=%0d",
                    w,
                ...
                $display(
                    "[TB_STAGE1M_HB] heartbeat interval=%0d MSM cycles",
    ...
    task automatic write_profile_csv;
        ...
        begin
            csv_fd = $fopen(
                "runs/full_axi_affine_1048576p_replay16_stageprofile_v1.csv",
                "w"
            );

            if (csv_fd == 0)
                $fatal(1, "[TB_STAGE1M_HB] could not open CSV");
        ...
        begin
            for (w = 0; w < NUM_WINDOWS; w = w + 1) begin
                if (point_in_count_win[w] != LOGICAL_POINTS_PER_WINDOW) begin
                    error_count++;
                    $error("[TB_STAGE1M_HB] w=%0d input exp=%0d got=%0d",
                ...
                if (mont_out_count_win[w] != LOGICAL_POINTS_PER_WINDOW) begin
                    error_count++;
                    $error("[TB_STAGE1M_HB] w=%0d mont exp=%0d got=%0d",
                           w, LOGICAL_POINTS_PER_WINDOW,
                ...
                if (direct_delta[w] !=
                    ...
                    error_count++;
                    $error("[TB_STAGE1M_HB] w=%0d direct exp=%0d got=%0d",
                ...
                if (mixed_delta[w] !=
                    ...
                    error_count++;
                    $error("[TB_STAGE1M_HB] w=%0d mixed exp=%0d got=%0d",
                ...
                if (zero_count_win[w] !=
                    ...
                    error_count++;
                    $error("[TB_STAGE1M_HB] w=%0d zero exp=%0d got=%0d",
                ...
                for (lane = 0; lane < NUM_LANES; lane = lane + 1) begin
                    if (lane_points_win[w][lane] !=
                        ...
                        error_count++;
                        $error(
                            "[TB_STAGE1M_HB] w=%0d lane=%0d exp=%0d got=%0d",
                            w, lane,
    ...
    initial begin : test_sequence
        ...
        if (LOGICAL_POINTS_PER_WINDOW !=
            BASE_POINTS_PER_WINDOW * REPLAY_FACTOR) begin
            $fatal(
                1,
                "[TB_STAGE1M_HB] replay geometry mismatch"
            );
        end

        if (MW_ORIGINAL_POINTS != LOGICAL_POINTS_PER_WINDOW) begin
            $fatal(
                1,
                "[TB_STAGE1M_HB] expected-file point count mismatch"
            );
        end
        ...
        if (error_count == 0) begin
            ...
            $display("============================================================");
            $display(
                "[TB_STAGE1M_HB] FULL 1,048,576-POINT REPLAY16 AXI AFFINE STAGE PROFILE PASSED"
            );
            $display("[TB_STAGE1M_HB] windows              = %0d",
                     NUM_WINDOWS);
            $display("[TB_STAGE1M_HB] original points      = %0d",
                     LOGICAL_POINTS_PER_WINDOW);
            $display("[TB_STAGE1M_HB] point-window records = %0d",
                     NUM_WINDOWS*LOGICAL_POINTS_PER_WINDOW);
            $display("[TB_STAGE1M_HB] total AXI beats      = %0d",
                     r_beat_count);
            $display("[TB_STAGE1M_HB] AXI bursts           = %0d",
                     ar_burst_count);
            $display("[TB_STAGE1M_HB] done MSM cycle       = %0d",
                     msm_cycle_count);
            $display(
                "[TB_STAGE1M_HB] CSV: runs/full_axi_affine_1048576p_replay16_stageprofile_v1.csv"
            );
            $display("============================================================");
        end else begin
            $fatal(1, "[TB_STAGE1M_HB] FAILED with %0d errors",
                   error_count);
        end
    ...
    initial begin
        #(64'd16000000000);
        $fatal(1, "[TB_STAGE1M_HB] Watchdog timeout");
    end
