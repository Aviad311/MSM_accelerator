...
module tb_msm_axi_affine_multiwindow_dualclk_16win_64p_stageprofile_v2;
    ...
    `include "vectors/full_axi_affine_64p_stageprofile_v1_expected.svh"
    ...
    initial begin : init_axi_memory
        ...
        $readmemh(
            "vectors/full_axi_affine_64p_stageprofile_v1.memh",
            axi_mem
        );
    end
        ...
        if (!axi_rst_n) begin
            ...
            if (m_axi_arvalid && m_axi_arready) begin
                ...
                if (m_axi_arsize !== 3'b110) begin
                    error_count++;
                    $error("[TB_STAGE64] bad ARSIZE=%0d", m_axi_arsize);
                end

                if (m_axi_arburst !== 2'b01) begin
                    error_count++;
                    $error("[TB_STAGE64] bad ARBURST=%0b", m_axi_arburst);
                end

                if (m_axi_araddr[5:0] != 0) begin
                    error_count++;
                    $error("[TB_STAGE64] unaligned ARADDR=%h", m_axi_araddr);
                end
            ...
            if (!m_axi_rvalid &&
                ...
                if ((mem_index < 0) || (mem_index >= TOTAL_BEATS)) begin
                    error_count++;
                    $error("[TB_STAGE64] invalid beat index=%0d", mem_index);
                    m_axi_rdata <= '0;
                end else begin
            ...
            if (m_axi_rvalid && m_axi_rready) begin
                ...
                if (m_axi_rlast !== (slave_beats_left_q == 1)) begin
                    error_count++;
                    $error("[TB_STAGE64] RLAST mismatch");
                end
                ...
                if (slave_beats_left_q == 1) begin
                    ...
                    if (gap_lfsr_q[8:6] == 3'b000) begin
                        ...
                        if ((mem_index < 0) ||
                            ...
                            error_count++;
                            $error("[TB_STAGE64] invalid next beat=%0d",
                                   mem_index);
                            m_axi_rdata <= '0;
                        end else begin
                            m_axi_rdata <= axi_mem[mem_index];
    ...
    always @(posedge msm_clk or negedge msm_rst_n) begin : msm_profile
        ...
        if (!msm_rst_n) begin
            ...
            if (busy && (window_index != last_seen_window)) begin
                ...
                $display(
                    "[TB_STAGE64] entered window=%0d base=%h msm_cycle=%0d",
                    w,
    ...
    task automatic write_profile_csv;
        ...
        begin
            csv_fd = $fopen(
                "runs/full_axi_affine_64p_stageprofile_v1.csv",
                "w"
            );

            if (csv_fd == 0)
                $fatal(1, "[TB_STAGE64] could not open CSV");
        ...
        begin
            for (w = 0; w < NUM_WINDOWS; w = w + 1) begin
                if (point_in_count_win[w] != LOGICAL_POINTS_PER_WINDOW) begin
                    error_count++;
                    $error("[TB_STAGE64] w=%0d input exp=%0d got=%0d",
                ...
                if (mont_out_count_win[w] != LOGICAL_POINTS_PER_WINDOW) begin
                    error_count++;
                    $error("[TB_STAGE64] w=%0d mont exp=%0d got=%0d",
                           w, LOGICAL_POINTS_PER_WINDOW,
                ...
                if (direct_delta[w] !=
                    ...
                    error_count++;
                    $error("[TB_STAGE64] w=%0d direct exp=%0d got=%0d",
                ...
                if (mixed_delta[w] !=
                    ...
                    error_count++;
                    $error("[TB_STAGE64] w=%0d mixed exp=%0d got=%0d",
                ...
                if (zero_count_win[w] !=
                    ...
                    error_count++;
                    $error("[TB_STAGE64] w=%0d zero exp=%0d got=%0d",
                ...
                for (lane = 0; lane < NUM_LANES; lane = lane + 1) begin
                    if (lane_points_win[w][lane] !=
                        ...
                        error_count++;
                        $error(
                            "[TB_STAGE64] w=%0d lane=%0d exp=%0d got=%0d",
                            w, lane,
    ...
    initial begin : test_sequence
        ...
        if (error_count == 0) begin
            ...
            $display("============================================================");
            $display(
                "[TB_STAGE64] FULL 64-POINT AXI AFFINE STAGE PROFILE PASSED"
            );
            $display("[TB_STAGE64] windows              = %0d",
                     NUM_WINDOWS);
            $display("[TB_STAGE64] original points      = %0d",
                     LOGICAL_POINTS_PER_WINDOW);
            $display("[TB_STAGE64] point-window records = %0d",
                     NUM_WINDOWS*LOGICAL_POINTS_PER_WINDOW);
            $display("[TB_STAGE64] total AXI beats      = %0d",
                     r_beat_count);
            $display("[TB_STAGE64] AXI bursts           = %0d",
                     ar_burst_count);
            $display("[TB_STAGE64] done MSM cycle       = %0d",
                     msm_cycle_count);
            $display(
                "[TB_STAGE64] CSV: runs/full_axi_affine_64p_stageprofile_v1.csv"
            );
            $display("============================================================");
        end else begin
            $fatal(1, "[TB_STAGE64] FAILED with %0d errors",
                   error_count);
        end
    ...
    initial begin
        #(64'd4000000000);
        $fatal(1, "[TB_STAGE64] Watchdog timeout");
    end
