...
module tb_pippenger_window_mem_stream_suite_4lane;
    ...
    pippenger_window_mem_stream_top_4lane #(
        .ADDR_W          (ADDR_W),
        .DATA_W          (DATA_W),
        .DEPTH           (DEPTH),
    ...
    task automatic run_case_stream(
        ...
        begin
            ...
            if (result_x !== exp_x || result_y !== exp_y || result_z !== exp_z) begin
                ...
                $fatal(1, "[TB] 4-lane suite stream mismatch error.");
            end else begin
                $display("[TB] %s PASSED latency=%0d cycles", test_name, latency);

                $display("[PERF4] %s expected_points        = %0d", test_name, perf_expected_points);
                $display("[PERF4] %s total_cycles           = %0d", test_name, perf_total_cycles);
                $display("[PERF4] %s busy_cycles            = %0d", test_name, perf_busy_cycles);
                $display("[PERF4] %s idle_cycles            = %0d", test_name, perf_idle_cycles);
                $display("[PERF4] %s input_valid_cycles     = %0d", test_name, perf_input_valid_cycles);
                $display("[PERF4] %s input_accept_count     = %0d", test_name, perf_input_accept_count);
                $display("[PERF4] %s input_stall_cycles     = %0d", test_name, perf_input_stall_cycles);
    ...
    initial begin
        ...
        $display("[TB] Starting 4-LANE SUITE pippenger_window_mem_stream_top validation");
        ...
        for (int case_idx = 0; case_idx < SUITE_NUM_CASES; case_idx++) begin
            $display("");
            $display("[TB] ==================================================");
            $display("[TB] Running 4-lane suite case %0d/%0d: %s",
        ...
        $display("");
        $display("[TB] All 4-lane suite vector stream verification cases PASSED.");
        #20;
        $finish;
    end
    ...
    initial begin
        #50000000;
        $display("[TB] ERROR: 4-lane simulation hit maximum watchdog limit timeout!");
        $finish;
    end
