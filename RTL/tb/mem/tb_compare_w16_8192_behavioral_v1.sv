...
module tb_compare_w16_8192_behavioral_v1;
    ...
    task automatic run_case(
        ...
        begin
            ...
            if (!reduce_seen) begin
                $fatal(
                    1,
                    "[TB_COMPARE_BEHAV] reduce_start was not observed."
                );
            end
            ...
            if (result_x !== suite_expected_x[case_idx] ||
                ...
                $display(
                    "[TB_COMPARE_BEHAV] %s FAILED",
                    suite_case_name[case_idx]
                );
                $display(
                    "[TB_COMPARE_BEHAV] EXPECTED X = %064h",
                    suite_expected_x[case_idx]
                );
                $display(
                    "[TB_COMPARE_BEHAV] GOT      X = %064h",
                    result_x
                );
                $display(
                    "[TB_COMPARE_BEHAV] EXPECTED Y = %064h",
                    suite_expected_y[case_idx]
                );
                $display(
                    "[TB_COMPARE_BEHAV] GOT      Y = %064h",
                    result_y
                );
                $display(
                    "[TB_COMPARE_BEHAV] EXPECTED Z = %064h",
                    suite_expected_z[case_idx]
                );
                $display(
                    "[TB_COMPARE_BEHAV] GOT      Z = %064h",
                    result_z
                );

                $fatal(
                    1,
                    "[TB_COMPARE_BEHAV] Result mismatch."
                );
            end
            ...
            if (input_accept_count != stream_size) begin
                $fatal(
                    1,
                    "[TB_COMPARE_BEHAV] Accepted-count mismatch: expected=%0d got=%0d",
                    stream_size,
                    input_accept_count
                );
            end
            ...
            $display("");
            $display(
                "[TB_COMPARE_BEHAV] %s PASSED",
                suite_case_name[case_idx]
            );
            $display(
                "[PERF_COMPARE_BEHAV] %s expected_points    = %0d",
                suite_case_name[case_idx],
                stream_size
            );
            $display(
                "[PERF_COMPARE_BEHAV] %s direct_write       = %0d",
                suite_case_name[case_idx],
                suite_direct_write_count[case_idx]
            );
            $display(
                "[PERF_COMPARE_BEHAV] %s mixed_add          = %0d",
                suite_case_name[case_idx],
                suite_mixed_add_count[case_idx]
            );
            $display(
                "[PERF_COMPARE_BEHAV] %s reduce_active      = %0d",
                suite_case_name[case_idx],
                suite_reduce_active_count[case_idx]
            );
            $display(
                "[PERF_COMPARE_BEHAV] %s build_cycles       = %0d",
                suite_case_name[case_idx],
                build_cycles
            );
            $display(
                "[PERF_COMPARE_BEHAV] %s reduce_cycles      = %0d",
                suite_case_name[case_idx],
                reduce_cycles
            );
            $display(
                "[PERF_COMPARE_BEHAV] %s total_cycles       = %0d",
                suite_case_name[case_idx],
                total_cycles
            );
            $display(
                "[PERF_COMPARE_BEHAV] %s build_percent      = %0.2f",
                suite_case_name[case_idx],
                100.0 * build_cycles / total_cycles
            );
            $display(
                "[PERF_COMPARE_BEHAV] %s reduce_percent     = %0.2f",
                suite_case_name[case_idx],
                100.0 * reduce_cycles / total_cycles
            );
            $display(
                "[PERF_COMPARE_BEHAV] %s input_valid_cycles = %0d",
                suite_case_name[case_idx],
                input_valid_cycles
            );
            $display(
                "[PERF_COMPARE_BEHAV] %s input_accept_count = %0d",
                suite_case_name[case_idx],
                input_accept_count
            );
            $display(
                "[PERF_COMPARE_BEHAV] %s input_stall_cycles = %0d",
                suite_case_name[case_idx],
                input_stall_cycles
            );

            $display(
                "[PERF_COMPARE_BEHAV] %s accept_rate_points_per_cycle = %0.6f",
                suite_case_name[case_idx],
                (input_valid_cycles == 0)
                    ? 0.0
            ...
            $display(
                "[PERF_COMPARE_BEHAV] %s effective_input_ii = %0.6f",
                suite_case_name[case_idx],
                (input_accept_count == 0)
                    ? 0.0
    ...
    initial begin
        ...
        $display("");
        $display(
            "[TB_COMPARE_BEHAV] Starting W=16 phase-separated performance suite"
        );
        $display(
            "[TB_COMPARE_BEHAV] ADDR_W=%0d DEPTH=%0d cases=%0d",
            ADDR_W,
            DEPTH,
            SUITE_NUM_CASES
        );
        ...
        if (SUITE_ADDR_W != ADDR_W)
            $fatal(1, "[TB_COMPARE_BEHAV] ADDR_W mismatch.");

        if (SUITE_DEPTH != DEPTH)
            $fatal(1, "[TB_COMPARE_BEHAV] DEPTH mismatch.");

        // Fair comparison: run only the shared 8192-point uniform case.
        if (SUITE_NUM_CASES < 3)
            $fatal(1, "[TB_COMPARE_BEHAV] Expected at least three suite cases.");

        $display("");
        $display(
            "[TB_COMPARE_BEHAV] Running shared comparison case: %s",
            suite_case_name[2]
        );
        ...
        $display("");
        $display(
            "[TB_COMPARE_BEHAV] 8192-point comparison case PASSED."
        );
    ...
    initial begin
        ...
        $fatal(
            1,
            "[TB_COMPARE_BEHAV] Watchdog timeout."
        );
    end
