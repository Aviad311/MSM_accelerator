...
module tb_bucket_build_mem_seq_4;
    ...
    task automatic check_bucket(
        ...
        logic [255:0] got_z;
        begin
            got_x = dut.u_update.u_bucket_mem.u_bucket_x_mem.mem[idx];
            got_y = dut.u_update.u_bucket_mem.u_bucket_y_mem.mem[idx];
            got_z = dut.u_update.u_bucket_mem.u_bucket_z_mem.mem[idx];
            ...
            if (got_x !== exp_x || got_y !== exp_y || got_z !== exp_z) begin
                ...
                $fatal(1, "[TB] bucket_build_mem_seq_4 bucket mismatch");
            end else begin
    ...
    initial begin
        ...
        $display("[TB] Starting bucket_build_mem_seq_4 test");
        ...
        //
        // Expected:
        // bucket1 = G + G = 2G Jacobian
        // bucket2 = 2G affine with Z=ONE_M
        // bucket3 = 3G affine with Z=ONE_M
        ...
        $display("[TB] tb_bucket_build_mem_seq_4 PASSED");
        #20;
        $finish;
    end
