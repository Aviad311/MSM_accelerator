...
module tb_pippenger_window_mem_stream_suite_8lane_pipeline_reduce4mul_overlap_w16_sram_macro_v1;
    ...
    pippenger_window_mem_stream_top_8lane_pipeline_reduce4mul_overlap_sram_macro_v1 #(
        .ADDR_W          (ADDR_W),
        .DATA_W          (DATA_W),
        .DEPTH           (DEPTH),
    ...
    task automatic run_case_stream(
        ...
        begin
            ...
            if (stream_size <= 0) begin
                $fatal(
                    1,
                    "[TB_W16_MACRO] %s has no input points.",
                    test_name
                );
            end
            ...
            if (result_x !== exp_x ||
                ...
                $display("");
                $display(
                    "[TB_W16_MACRO] %s FAILED",
                    test_name
                );

                $display(
                    "[TB_W16_MACRO] EXPECTED X = %064h",
                    exp_x
                );
                $display(
                    "[TB_W16_MACRO] GOT      X = %064h",
                    result_x
                );

                $display(
                    "[TB_W16_MACRO] EXPECTED Y = %064h",
                    exp_y
                );
                $display(
                    "[TB_W16_MACRO] GOT      Y = %064h",
                    result_y
                );

                $display(
                    "[TB_W16_MACRO] EXPECTED Z = %064h",
                    exp_z
                );
                $display(
                    "[TB_W16_MACRO] GOT      Z = %064h",
                    result_z
                );

                $fatal(
                    1,
                    "[TB_W16_MACRO] W=16 smoke-suite mismatch."
                );
            end else begin
                $display(
                    "[TB_W16_MACRO] %s PASSED latency=%0d cycles",
                    test_name,
                    latency
                );

                $display(
                    "[PERF_W16_MACRO] %s expected_points    = %0d",
                    test_name,
                    perf_expected_points
                );

                $display(
                    "[PERF_W16_MACRO] %s total_cycles       = %0d",
                    test_name,
                    perf_total_cycles
                );

                $display(
                    "[PERF_W16_MACRO] %s busy_cycles        = %0d",
                    test_name,
                    perf_busy_cycles
                );

                $display(
                    "[PERF_W16_MACRO] %s idle_cycles        = %0d",
                    test_name,
                    perf_idle_cycles
                );

                $display(
                    "[PERF_W16_MACRO] %s input_valid_cycles = %0d",
                    test_name,
                    perf_input_valid_cycles
                );

                $display(
                    "[PERF_W16_MACRO] %s input_accept_count = %0d",
                    test_name,
                    perf_input_accept_count
                );

                $display(
                    "[PERF_W16_MACRO] %s input_stall_cycles = %0d",
                    test_name,
                    perf_input_stall_cycles
                );
                ...
                if (perf_input_accept_count != perf_expected_points) begin
                    $fatal(
                        1,
                        "[TB_W16_MACRO] %s accepted-count mismatch: expected=%0d got=%0d",
                        test_name,
                        perf_expected_points,
                        perf_input_accept_count
                    );
    ...
    initial begin
        ...
        $display("");
        $display(
            "[TB_W16_MACRO] Starting W=16 8-LANE SRAM-MACRO PIPELINE + REDUCE 4MUL OVERLAP suite"
        );

        $display(
            "[TB_W16_MACRO] Configuration: ADDR_W=%0d DEPTH=%0d SRAM_RD_LATENCY=%0d",
            ADDR_W,
            DEPTH,
            SRAM_RD_LATENCY
        );
        ...
        $display(
            "[TB_W16_MACRO] Bank configuration: NUM_BANKS=8 BANK_ADDR_W=%0d BANK_DEPTH=%0d",
            ADDR_W - 3,
            1 << (ADDR_W - 3)
        );

        $display(
            "[TB_W16_MACRO] Suite: cases=%0d max_points=%0d suite_addr_w=%0d suite_depth=%0d",
            SUITE_NUM_CASES,
            SUITE_MAX_POINTS,
            SUITE_ADDR_W,
            SUITE_DEPTH
        ...
        if (ADDR_W != 16) begin
            $fatal(
                1,
                "[TB_W16_MACRO] This testbench requires ADDR_W=16, got %0d.",
                ADDR_W
            );
        end
        ...
        if (SUITE_ADDR_W != ADDR_W) begin
            $fatal(
                1,
                "[TB_W16_MACRO] Suite ADDR_W mismatch: suite=%0d tb=%0d",
                SUITE_ADDR_W,
                ADDR_W
            );
        end
        ...
        if (SUITE_DEPTH != DEPTH) begin
            $fatal(
                1,
                "[TB_W16_MACRO] Suite DEPTH mismatch: suite=%0d tb=%0d",
                SUITE_DEPTH,
                DEPTH
            );
        end
        ...
        for (
            ...
            $display(
                "[TB_W16_MACRO] Running case %0d/%0d: %s",
                case_idx + 1,
                SUITE_NUM_CASES,
            ...
            $display(
                "[TB_W16_MACRO] Python stats: num_points=%0d direct_write=%0d mixed_add=%0d skipped_zero=%0d reduce_scan=%0d reduce_active=%0d",
            ...
            if (suite_reduce_scan_count[case_idx] != DEPTH - 1) begin
                $fatal(
                    1,
                    "[TB_W16_MACRO] Case %s has invalid reduce_scan_count=%0d expected=%0d",
                    suite_case_name[case_idx],
                    suite_reduce_scan_count[case_idx],
                    DEPTH - 1
                );
        ...
        $display("");
        $display(
            "[TB_W16_MACRO] All W=16 8-lane PIPELINE + REDUCE 4MUL OVERLAP smoke cases PASSED."
        );
    ...
    initial begin
        ...
        $display("");
        $display(
            "[TB_W16_MACRO] ERROR: W=16 smoke-suite watchdog timeout!"
        );

        $fatal(
            1,
            "[TB_W16_MACRO] Simulation exceeded the watchdog limit."
        );
    end
