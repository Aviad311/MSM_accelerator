...
module tb_pippenger_window_mem_stream_suite_8lane_pipeline_reduce4mul_overlap;
    ...
    task automatic run_case_stream(
        ...
        begin
            ...
            if (result_x !== exp_x ||
                ...
                $display(
                    "[PERF8_PIPE_R4_OV] %s expected_points        = %0d",
                    test_name,
                    perf_expected_points
                );

                $display(
                    "[PERF8_PIPE_R4_OV] %s total_cycles           = %0d",
                    test_name,
                    perf_total_cycles
                );

                $display(
                    "[PERF8_PIPE_R4_OV] %s busy_cycles            = %0d",
                    test_name,
                    perf_busy_cycles
                );

                $display(
                    "[PERF8_PIPE_R4_OV] %s idle_cycles            = %0d",
                    test_name,
                    perf_idle_cycles
                );

                $display(
                    "[PERF8_PIPE_R4_OV] %s input_valid_cycles     = %0d",
                    test_name,
                    perf_input_valid_cycles
                );

                $display(
                    "[PERF8_PIPE_R4_OV] %s input_accept_count     = %0d",
                    test_name,
                    perf_input_accept_count
                );

                $display(
                    "[PERF8_PIPE_R4_OV] %s input_stall_cycles     = %0d",
                    test_name,
                    perf_input_stall_cycles
                );
                ...
                if (perf_input_accept_count != perf_expected_points) begin
                    $display(
                        "[PERF8_PIPE_R4_OV_WARN] %s accepted mismatch: expected=%0d got=%0d",
                        test_name,
                        perf_expected_points,
                        perf_input_accept_count
                    );
    ...
    initial begin
        ...
        $display(
            "[TB] Starting 8-LANE PIPELINE + REDUCE 4MUL OVERLAP SUITE validation"
        );
        ...
        $display("");
        $display(
            "[TB] All 8-lane PIPELINE + REDUCE 4MUL OVERLAP suite cases PASSED."
        );
