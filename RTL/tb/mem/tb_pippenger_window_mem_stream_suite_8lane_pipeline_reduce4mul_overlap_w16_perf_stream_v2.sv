...
module tb_pippenger_window_mem_stream_suite_8lane_pipeline_reduce4mul_overlap_w16_perf_stream_v2;
    ...
    task automatic run_case(
        ...
        begin
            ...
            if (!reduce_seen) begin
                $fatal(
                    1,
                    "[TB_W16_STREAM_V2] reduce_start was not observed."
                );
            end
            ...
            if (result_x !== suite_expected_x[case_idx] ||
                ...
                $display(
                    "[TB_W16_STREAM_V2] %s FAILED",
                    suite_case_name[case_idx]
                );
                $display(
                    "[TB_W16_STREAM_V2] EXPECTED X = %064h",
                    suite_expected_x[case_idx]
                );
                $display(
                    "[TB_W16_STREAM_V2] GOT      X = %064h",
                    result_x
                );
                $display(
                    "[TB_W16_STREAM_V2] EXPECTED Y = %064h",
                    suite_expected_y[case_idx]
                );
                $display(
                    "[TB_W16_STREAM_V2] GOT      Y = %064h",
                    result_y
                );
                $display(
                    "[TB_W16_STREAM_V2] EXPECTED Z = %064h",
                    suite_expected_z[case_idx]
                );
                $display(
                    "[TB_W16_STREAM_V2] GOT      Z = %064h",
                    result_z
                );

                $fatal(
                    1,
                    "[TB_W16_STREAM_V2] Result mismatch."
                );
            end
            ...
            if (input_accept_count != stream_size) begin
                $fatal(
                    1,
                    "[TB_W16_STREAM_V2] Accepted-count mismatch: expected=%0d got=%0d",
                    stream_size,
                    input_accept_count
                );
            end
            ...
            $display("");
            $display(
                "[TB_W16_STREAM_V2] %s PASSED",
                suite_case_name[case_idx]
            );
            $display(
                "[PERF_W16_STREAM_V2] %s expected_points    = %0d",
                suite_case_name[case_idx],
                stream_size
            );
            $display(
                "[PERF_W16_STREAM_V2] %s direct_write       = %0d",
                suite_case_name[case_idx],
                suite_direct_write_count[case_idx]
            );
            $display(
                "[PERF_W16_STREAM_V2] %s mixed_add          = %0d",
                suite_case_name[case_idx],
                suite_mixed_add_count[case_idx]
            );
            $display(
                "[PERF_W16_STREAM_V2] %s reduce_active      = %0d",
                suite_case_name[case_idx],
                suite_reduce_active_count[case_idx]
            );
            $display(
                "[PERF_W16_STREAM_V2] %s build_cycles       = %0d",
                suite_case_name[case_idx],
                build_cycles
            );
            $display(
                "[PERF_W16_STREAM_V2] %s reduce_cycles      = %0d",
                suite_case_name[case_idx],
                reduce_cycles
            );
            $display(
                "[PERF_W16_STREAM_V2] %s total_cycles       = %0d",
                suite_case_name[case_idx],
                total_cycles
            );
            $display(
                "[PERF_W16_STREAM_V2] %s build_percent      = %0.2f",
                suite_case_name[case_idx],
                100.0 * build_cycles / total_cycles
            );
            $display(
                "[PERF_W16_STREAM_V2] %s reduce_percent     = %0.2f",
                suite_case_name[case_idx],
                100.0 * reduce_cycles / total_cycles
            );
            $display(
                "[PERF_W16_STREAM_V2] %s input_valid_cycles = %0d",
                suite_case_name[case_idx],
                input_valid_cycles
            );
            $display(
                "[PERF_W16_STREAM_V2] %s input_accept_count = %0d",
                suite_case_name[case_idx],
                input_accept_count
            );
            $display(
                "[PERF_W16_STREAM_V2] %s input_stall_cycles = %0d",
                suite_case_name[case_idx],
                input_stall_cycles
            );

            $display(
                "[PERF_W16_STREAM_V2] %s accept_rate_points_per_cycle = %0.6f",
                suite_case_name[case_idx],
                (input_valid_cycles == 0)
                    ? 0.0
            ...
            $display(
                "[PERF_W16_STREAM_V2] %s effective_input_ii = %0.6f",
                suite_case_name[case_idx],
                (input_accept_count == 0)
                    ? 0.0
    ...
    initial begin
        ...
        $display("");
        $display(
            "[TB_W16_STREAM_V2] Starting W=16 phase-separated performance suite"
        );
        $display(
            "[TB_W16_STREAM_V2] ADDR_W=%0d DEPTH=%0d cases=%0d",
            ADDR_W,
            DEPTH,
            SUITE_NUM_CASES
        );
        ...
        if (SUITE_ADDR_W != ADDR_W)
            $fatal(1, "[TB_W16_STREAM_V2] ADDR_W mismatch.");

        if (SUITE_DEPTH != DEPTH)
            $fatal(1, "[TB_W16_STREAM_V2] DEPTH mismatch.");
        ...
        for (int case_idx = 0;
            ...
            $display("");
            $display(
                "[TB_W16_STREAM_V2] Running case %0d/%0d: %s",
                case_idx + 1,
                SUITE_NUM_CASES,
                suite_case_name[case_idx]
            );
        ...
        $display("");
        $display(
            "[TB_W16_STREAM_V2] All W=16 performance cases PASSED."
        );
    ...
    initial begin
        ...
        $fatal(
            1,
            "[TB_W16_STREAM_V2] Watchdog timeout."
        );
    end
