...
module tb_pippenger_window_mem_stream_suite_2lane;
    ...
    pippenger_window_mem_stream_top_2lane #(
        .ADDR_W          (ADDR_W),
        .DATA_W          (DATA_W),
        .DEPTH           (DEPTH),
    ...
    task automatic run_case_stream(
        ...
        begin
            ...
            if (result_x !== exp_x || result_y !== exp_y || result_z !== exp_z) begin
                ...
                $fatal(1, "[TB] 2-lane suite stream mismatch error.");
            end else begin
                $display("[TB] %s PASSED latency=%0d cycles", test_name, latency);

                $display("[PERF2] %s expected_points        = %0d", test_name, perf_expected_points);
                $display("[PERF2] %s total_cycles           = %0d", test_name, perf_total_cycles);
                $display("[PERF2] %s busy_cycles            = %0d", test_name, perf_busy_cycles);
                $display("[PERF2] %s idle_cycles            = %0d", test_name, perf_idle_cycles);
                $display("[PERF2] %s input_valid_cycles     = %0d", test_name, perf_input_valid_cycles);
                $display("[PERF2] %s input_accept_count     = %0d", test_name, perf_input_accept_count);
                $display("[PERF2] %s input_stall_cycles     = %0d", test_name, perf_input_stall_cycles);
    ...
    initial begin
        ...
        $display("[TB] Starting 2-LANE SUITE pippenger_window_mem_stream_top validation");
        ...
        for (int case_idx = 0; case_idx < SUITE_NUM_CASES; case_idx++) begin
            $display("");
            $display("[TB] ==================================================");
            $display("[TB] Running 2-lane suite case %0d/%0d: %s",
        ...
        $display("");
        $display("[TB] All 2-lane suite vector stream verification cases PASSED.");
        #20;
        $finish;
    end
    ...
    initial begin
        #50000000;
        $display("[TB] ERROR: 2-lane simulation hit maximum watchdog limit timeout!");
        $finish;
    end
