...
// ============================================================================
// W=16 Build-only performance suite.
//
...
module tb_pippenger_window_mem_stream_suite_8lane_pipeline_w16_build_only;
    ...
    task automatic run_build_case(
        ...
        begin
            ...
            if (input_accept_count != stream_size) begin
                $fatal(
                    1,
                    "[TB_W16_BUILD] Accepted-count mismatch: expected=%0d got=%0d",
                    stream_size,
                    input_accept_count
                );
            end
            ...
            $display("");
            $display(
                "[TB_W16_BUILD] %s BUILD COMPLETED",
                suite_case_name[case_idx]
            );
            $display(
                "[PERF_W16_BUILD] %s expected_points      = %0d",
                suite_case_name[case_idx],
                stream_size
            );
            $display(
                "[PERF_W16_BUILD] %s active_bucket_count  = %0d",
                suite_case_name[case_idx],
                suite_active_bucket_count[case_idx]
            );
            $display(
                "[PERF_W16_BUILD] %s direct_write_count   = %0d",
                suite_case_name[case_idx],
                suite_direct_write_count[case_idx]
            );
            $display(
                "[PERF_W16_BUILD] %s mixed_add_count      = %0d",
                suite_case_name[case_idx],
                suite_mixed_add_count[case_idx]
            );
            $display(
                "[PERF_W16_BUILD] %s build_cycles         = %0d",
                suite_case_name[case_idx],
                build_cycles
            );
            $display(
                "[PERF_W16_BUILD] %s points_per_cycle     = %0.6f",
                suite_case_name[case_idx],
                points_per_cycle
            );
            $display(
                "[PERF_W16_BUILD] %s cycles_per_point     = %0.6f",
                suite_case_name[case_idx],
                cycles_per_point
            );
            $display(
                "[PERF_W16_BUILD] %s mixed_adds_per_cycle = %0.6f",
                suite_case_name[case_idx],
                mixed_adds_per_cycle
            );
            $display(
                "[PERF_W16_BUILD] %s input_valid_cycles   = %0d",
                suite_case_name[case_idx],
                input_valid_cycles
            );
            $display(
                "[PERF_W16_BUILD] %s input_accept_count   = %0d",
                suite_case_name[case_idx],
                input_accept_count
            );
            $display(
                "[PERF_W16_BUILD] %s input_stall_cycles   = %0d",
                suite_case_name[case_idx],
                input_stall_cycles
            );
    ...
    initial begin
        ...
        $display("");
        $display(
            "[TB_W16_BUILD] Starting W=16 Build-only dense-workload suite"
        );
        $display(
            "[TB_W16_BUILD] ADDR_W=%0d DEPTH=%0d cases=%0d",
            ADDR_W,
            DEPTH,
            SUITE_NUM_CASES
        );
        ...
        if (SUITE_ADDR_W != ADDR_W)
            $fatal(1, "[TB_W16_BUILD] ADDR_W mismatch.");

        if (SUITE_DEPTH != DEPTH)
            $fatal(1, "[TB_W16_BUILD] DEPTH mismatch.");
        ...
        for (int case_idx = 0;
            ...
            $display("");
            $display(
                "[TB_W16_BUILD] Running case %0d/%0d: %s",
                case_idx + 1,
                SUITE_NUM_CASES,
                suite_case_name[case_idx]
            );
        ...
        $display("");
        $display(
            "[TB_W16_BUILD] All W=16 Build-only cases COMPLETED."
        );
