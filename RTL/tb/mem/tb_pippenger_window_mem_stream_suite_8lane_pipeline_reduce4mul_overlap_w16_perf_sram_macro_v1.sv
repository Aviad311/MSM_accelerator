...
module tb_pippenger_window_mem_stream_suite_8lane_pipeline_reduce4mul_overlap_w16_perf_sram_macro_v1;
    ...
    task automatic run_case(
        ...
        begin
            ...
            if (!reduce_seen) begin
                $fatal(
                    1,
                    "[TB_W16_PERF_MACRO] reduce_start was not observed."
                );
            end
            ...
            if (result_x !== suite_expected_x[case_idx] ||
                ...
                $display(
                    "[TB_W16_PERF_MACRO] %s FAILED",
                    suite_case_name[case_idx]
                );
                $display(
                    "[TB_W16_PERF_MACRO] EXPECTED X = %064h",
                    suite_expected_x[case_idx]
                );
                $display(
                    "[TB_W16_PERF_MACRO] GOT      X = %064h",
                    result_x
                );
                $display(
                    "[TB_W16_PERF_MACRO] EXPECTED Y = %064h",
                    suite_expected_y[case_idx]
                );
                $display(
                    "[TB_W16_PERF_MACRO] GOT      Y = %064h",
                    result_y
                );
                $display(
                    "[TB_W16_PERF_MACRO] EXPECTED Z = %064h",
                    suite_expected_z[case_idx]
                );
                $display(
                    "[TB_W16_PERF_MACRO] GOT      Z = %064h",
                    result_z
                );

                $fatal(
                    1,
                    "[TB_W16_PERF_MACRO] Result mismatch."
                );
            end
            ...
            if (input_accept_count != stream_size) begin
                $fatal(
                    1,
                    "[TB_W16_PERF_MACRO] Accepted-count mismatch: expected=%0d got=%0d",
                    stream_size,
                    input_accept_count
                );
            end
            ...
            $display("");
            $display(
                "[TB_W16_PERF_MACRO] %s PASSED",
                suite_case_name[case_idx]
            );
            $display(
                "[PERF_W16_PHASE_MACRO] %s expected_points    = %0d",
                suite_case_name[case_idx],
                stream_size
            );
            $display(
                "[PERF_W16_PHASE_MACRO] %s direct_write       = %0d",
                suite_case_name[case_idx],
                suite_direct_write_count[case_idx]
            );
            $display(
                "[PERF_W16_PHASE_MACRO] %s mixed_add          = %0d",
                suite_case_name[case_idx],
                suite_mixed_add_count[case_idx]
            );
            $display(
                "[PERF_W16_PHASE_MACRO] %s reduce_active      = %0d",
                suite_case_name[case_idx],
                suite_reduce_active_count[case_idx]
            );
            $display(
                "[PERF_W16_PHASE_MACRO] %s build_cycles       = %0d",
                suite_case_name[case_idx],
                build_cycles
            );
            $display(
                "[PERF_W16_PHASE_MACRO] %s reduce_cycles      = %0d",
                suite_case_name[case_idx],
                reduce_cycles
            );
            $display(
                "[PERF_W16_PHASE_MACRO] %s total_cycles       = %0d",
                suite_case_name[case_idx],
                total_cycles
            );
            $display(
                "[PERF_W16_PHASE_MACRO] %s build_percent      = %0.2f",
                suite_case_name[case_idx],
                100.0 * build_cycles / total_cycles
            );
            $display(
                "[PERF_W16_PHASE_MACRO] %s reduce_percent     = %0.2f",
                suite_case_name[case_idx],
                100.0 * reduce_cycles / total_cycles
            );
            $display(
                "[PERF_W16_PHASE_MACRO] %s input_valid_cycles = %0d",
                suite_case_name[case_idx],
                input_valid_cycles
            );
            $display(
                "[PERF_W16_PHASE_MACRO] %s input_accept_count = %0d",
                suite_case_name[case_idx],
                input_accept_count
            );
            $display(
                "[PERF_W16_PHASE_MACRO] %s input_stall_cycles = %0d",
                suite_case_name[case_idx],
                input_stall_cycles
            );
    ...
    initial begin
        ...
        $display("");
        $display(
            "[TB_W16_PERF_MACRO] Starting W=16 phase-separated performance suite"
        );
        $display(
            "[TB_W16_PERF_MACRO] ADDR_W=%0d DEPTH=%0d cases=%0d",
            ADDR_W,
            DEPTH,
            SUITE_NUM_CASES
        );
        ...
        if (SUITE_ADDR_W != ADDR_W)
            $fatal(1, "[TB_W16_PERF_MACRO] ADDR_W mismatch.");

        if (SUITE_DEPTH != DEPTH)
            $fatal(1, "[TB_W16_PERF_MACRO] DEPTH mismatch.");
        ...
        for (int case_idx = 0;
            ...
            $display("");
            $display(
                "[TB_W16_PERF_MACRO] Running case %0d/%0d: %s",
                case_idx + 1,
                SUITE_NUM_CASES,
                suite_case_name[case_idx]
            );
        ...
        $display("");
        $display(
            "[TB_W16_PERF_MACRO] All W=16 performance cases PASSED."
        );
    ...
    initial begin
        ...
        $fatal(
            1,
            "[TB_W16_PERF_MACRO] Watchdog timeout."
        );
    end
